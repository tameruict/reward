...
import { OmoCaptchaClient } from '../../../util/OmoCaptcha'
...
export class FuncaptchaLogin {
    ...
    async isPresent(page: Page): Promise<boolean> {
        const visibleHostFrame = await page
            .locator(
                'iframe[src*="arkoselabs" i], iframe[src*="funcaptcha" i], iframe[title*="verification challenge" i], iframe[data-e2e="enforcement-frame"], iframe#enforcementFrame'
            )
            .filter({ visible: true })
            .count()
            .catch(() => 0)

        if (visibleHostFrame > 0) return true

        for (const frame of page.frames()) {
            if (frame.isDetached() || !ARKOSE_URL_PATTERN.test(frame.url())) continue
            if (await this.frameHasVisibleContent(frame)) return true
        }
        return false
    }
    ...
    async solve(page: Page): Promise<void> {
        ...
        const client = OmoCaptchaClient.fromEnvironment(this.bot.http)
        if (!client) {
            throw new Error(
                'FunCaptcha detected but OMOCaptcha is not configured. Add OMOCAPTCHA_API_KEY to .env and restart.'
            )
        }
        ...
        for (let round = 1; round <= 10; round++) {
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'CAPTCHA',
                `OMOCaptcha returned image index ${answerIndex}; moving carousel ${answerIndex - 1} time(s)`
            )
    ...
    private async waitForPuzzle(page: Page, timeoutMs: number): Promise<PuzzleSnapshot | null> {
        const deadline = Date.now() + timeoutMs

        while (Date.now() < deadline) {
            const challengeFrames: Frame[] = []
            for (const frame of [...page.frames()].reverse()) {
                if (frame.isDetached() || !(await this.frameHasVisibleContent(frame))) continue
                if (ARKOSE_URL_PATTERN.test(frame.url()) || (await this.frameLooksLikeChallenge(frame))) {
                    challengeFrames.push(frame)
                }
            }

            let sharedQuestion: string | null = null
            for (const frame of challengeFrames) {
                sharedQuestion = await this.readQuestion(frame)
                if (sharedQuestion) break
            }
            ...
            for (const frame of challengeFrames) {
                ...
                if (question) return { frame, imageBase64, question }
            }

            if (await this.isComplete(page)) return null
    ...
    private async readQuestion(frame: Frame): Promise<string | null> {
        return frame
            .evaluate(() => {
    ...
    private async readOriginalImage(frame: Frame): Promise<string | null> {
        return frame
            .evaluate(async () => {
                ...
                const toBase64 = async (url: string): Promise<string | null> => {
                    if (url.startsWith('data:image/')) return url.split(',', 2)[1] ?? null
                    if (!url) return null
                ...
                const backgroundElements = Array.from(document.querySelectorAll<HTMLElement>('div'))
                    .filter(isVisible)
                    .map(element => {
                        const rect = element.getBoundingClientRect()
    ...
    private async applyAnswer(frame: Frame, answerIndex: number): Promise<void> {
        for (let click = 1; click < answerIndex; click++) {
            const rightArrow = await this.findRightArrow(frame)
            if (!rightArrow) throw new Error('FunCaptcha right-arrow button was not found')
            await rightArrow.click()
            await this.bot.utils.wait(250)
        }

        const submit = await this.findButtonInHierarchy(
            frame,
            [
        ...
        if (!submit) throw new Error('FunCaptcha submit button was not found')
        await submit.click()
    }

    private async findRightArrow(frame: Frame): Promise<Locator | null> {
        const selectors = [
        ...
        for (const candidateFrame of this.frameHierarchy(frame)) {
            const visibleButtons: Array<{ locator: Locator; x: number; y: number }> = []
    ...
    private async clickStartButton(page: Page): Promise<void> {
        for (const frame of [...page.frames()].reverse()) {
            if (frame.isDetached() || !ARKOSE_URL_PATTERN.test(frame.url())) continue
            const startButton = await this.findButton(
                frame,
                ['[data-e2e="start-button"]', 'button[aria-label*="start" i]'],
                START_PATTERN
            )
            if (startButton) {
    ...
    private async findButton(frame: Frame, selectors: string[], namePattern: RegExp): Promise<Locator | null> {
        ...
        for (let index = 0; index < (await buttons.count()); index++) {
            ...
            if (namePattern.test(`${name ?? ''} ${text}`)) return candidate
        }
        return null
    }

    private async findButtonInHierarchy(
        frame: Frame,
        selectors: string[],
        namePattern: RegExp
    ...
    private frameHierarchy(frame: Frame): Frame[] {
        const frames: Frame[] = []
    ...
    private async isComplete(page: Page): Promise<boolean> {
        for (const frame of page.frames()) {
            if (frame.isDetached() || !ARKOSE_URL_PATTERN.test(frame.url())) continue
            const text = await frame.locator('body').innerText({ timeout: 500 }).catch(() => '')
            if (COMPLETE_PATTERN.test(text)) return true
        }
        return false
    }

    private async frameHasVisibleContent(frame: Frame): Promise<boolean> {
        try {
    ...
    private async frameLooksLikeChallenge(frame: Frame): Promise<boolean> {
