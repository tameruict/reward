...
export class UrlReward extends Workers {
    public async doUrlReward(promotion: BasePromotion) {
        ...
        const live =
            typeof this.bot.browser.func.ensureOffer === 'function'
        ...
        if (live?.isCompleted || live?.isLocked) {
            this.bot.logger.warn(
                this.bot.isMobile,
                'URL-REWARD',
                `Skipping ${offerId}: live offer is completed or locked`
            )
            return
        }
        ...
        const isPromotional =
            live?.isPromotional ?? String(promotion.attributes?.promotional ?? '').toLowerCase() === 'true'

        if (this.bot.config.skipNonPointTasks && this.isNonCrediting(points, promotionSubtype, title)) {
            this.bot.logger.info(
                this.bot.isMobile,
                'URL-REWARD',
                `Skipping ${offerId}: awards no points (points=${points}${promotionSubtype ? ` subtype=${promotionSubtype}` : ''}) - likely a free trial/non-crediting offer. Set skipNonPointTasks=false to attempt anyway.`
            )
            return
        }
        ...
        try {
            const routerStateTree =
                this.bot.browser.react?.routerStateTree?.('dashboard') ?? this.bot.nextRouterStateTree ?? ''
            ...
            if (!result.acknowledged) {
                ...
                if (refreshedHash) body[0] = refreshedHash
