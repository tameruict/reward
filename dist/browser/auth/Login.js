...
class Login {
    ...
    constructor(bot) {
        ...
        this.funcaptchaLogin = new FuncaptchaLogin_1.FuncaptchaLogin(this.bot);
    }
    async login(page, account) {
        try {
            ...
            await page
                .goto(urls_1.URLs.rewards.createUser, {
    ...
    async detectCurrentState(page, account) {
        ...
        if (url.hostname === 'rewards.bing.com') {
            this.bot.logger.debug(this.bot.isMobile, 'DETECT-STATE', 'Rewards host reached; dashboard validation pending');
            return 'LOGGED_IN';
        }
        const stateChecks = [
        ...
        if (foundStates.includes('ERROR_ALERT')) {
            ...
            this.bot.logger.debug(this.bot.isMobile, 'DETECT-STATE', `ERROR_ALERT found - hostname: ${url.hostname}, has 2FA: ${foundStates.includes('2FA_TOTP')}, treating as real: ${errorIsReal}`);
            if (errorIsReal)
                return 'ERROR_ALERT';
    ...
    async waitForIdle(page, note, timeout = 5000) {
        await page.waitForLoadState('networkidle', { timeout }).catch(() => {
            this.bot.logger.debug(this.bot.isMobile, 'LOGIN', `Network idle timeout: ${note}`);
        });
    }
    async tryClick(page, selector, label, timeout = 2000) {
    ...
    async handleState(state, page, account) {
        ...
        switch (state) {
            ...
            case 'CHROMEWEBDATA_ERROR': {
                ...
                const destinations = [urls_1.REWARDS_BASE_URL, urls_1.URLs.auth.loginLive];
                const failures = [];
                for (const destination of destinations) {
                    ...
                    try {
                        ...
                        this.bot.logger.info(this.bot.isMobile, 'LOGIN', `Recovery navigation reached ${currentUrl.hostname} (HTTP ${response.status()})`);
                        return true;
                    }
                    catch (error) {
                ...
                throw new Error(`Unable to recover from the Chromium network error page. Check or replace the configured proxy. ${failures.join('; ')}`);
            }
            case '2FA_TOTP': {
            ...
            case 'UNKNOWN': {
                ...
                if (this.bot.config.errorDiagnostics && !this.capturedUnknownUrls.has(rawUrl)) {
                    ...
                    await (0, ErrorDiagnostic_1.unknownPageDiagnostic)(page, {
    ...
    async finalizeLogin(page, account) {
        ...
        this.bot.logger.debug(this.bot.isMobile, 'LOGIN', `Saving session | cookies=${storageState.cookies.length} | origins=${storageState.origins.length}`);
        (0, SessionStore_1.saveStorageState)(this.bot.config.sessionPath, account.email, this.bot.isMobile, storageState);
        this.bot.logger.info(this.bot.isMobile, 'LOGIN', 'Login completed, session saved');
    }
    async verifyBingSession(page, account) {
        const url = urls_1.URLs.auth.bingSignIn;
        const loopMax = 5;
    ...
    async getRewardsSession(page, account) {
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    if (!(error instanceof BrowserFunc_1.RewardsAuthenticationRequiredError))
                        throw error;
                    ...
                    this.bot.logger.warn(this.bot.isMobile, 'GET-REWARD-SESSION', `Rewards authentication required at ${error.destination}; resuming Microsoft login flow once`);
            ...
            if (!actionsCount) {
                this.bot.logger.warn(this.bot.isMobile, 'GET-REWARD-SESSION', 'No action ids resolved - server-action calls (report/streak protection) will be skipped this run');
            }
            if (!snapshot || !snapshot.offers.length) {
                this.bot.logger.warn(this.bot.isMobile, 'GET-REWARD-SESSION', 'Offer snapshot empty - the optional /earn request may not have returned the RSC payload');
            }
            this.bot.logger.info(this.bot.isMobile, 'GET-REWARD-SESSION', `Context ready | actions=${actionsCount} | reportable=${reportableCount} | available=${availablePoints}`);
        }
        catch (error) {
            this.bot.logger.error(this.bot.isMobile, 'GET-REWARD-SESSION', `Failed to acquire rewards context: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    async runLoginStateMachine(page, account, phase) {
        ...
        while (iteration < maxIterations) {
            if (page.isClosed())
                throw new Error(`Page closed unexpectedly during ${phase}`);
            iteration++;
            this.bot.logger.debug(this.bot.isMobile, 'LOGIN', `State check iteration ${iteration}/${maxIterations} | phase=${phase}`);
            ...
            if (state === previousState && state !== 'LOGGED_IN' && state !== 'UNKNOWN') {
                ...
                if (sameStateCount >= 4) {
                    if (state === 'CHROMEWEBDATA_ERROR') {
                        throw new Error(`Chromium remained on its network error page during ${phase}. Check or replace the configured proxy.`);
                    }
                    this.bot.logger.warn(this.bot.isMobile, 'LOGIN', `Stuck in state "${state}" for 4 loops during ${phase}, refreshing page`);
