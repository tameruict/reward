...
class CodeLogin {
    ...
    emailInputSelector = '[data-testid="proof-confirmation"]';
    maxManualSeconds = 60;
    maxManualAttempts = 5;
    constructor(bot) {
        this.bot = bot;
    ...
    async fillEmail(page, email) {
        try {
            ...
            this.bot.logger.warn(this.bot.isMobile, 'LOGIN-CODE', `Failed to fill email input: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    async handle(page) {
        try {
            ...
            if (emailProofInput) {
                ...
                this.bot.logger.info(this.bot.isMobile, 'LOGIN-CODE', 'Email verification challenge detected, awaiting email address');
                ...
                for (let attempt = 1; attempt <= this.maxManualAttempts; attempt++) {
                    ...
                    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
                        this.bot.logger.warn(this.bot.isMobile, 'LOGIN-CODE', `Invalid or missing email (attempt ${attempt}/${this.maxManualAttempts}) | input length=${email?.length}`);
                    ...
                    if (!filled) {
                        this.bot.logger.error(this.bot.isMobile, 'LOGIN-CODE', `Unable to fill email input (attempt ${attempt}/${this.maxManualAttempts})`);
                        if (attempt === this.maxManualAttempts) {
                    ...
                    const emailError = await (0, LoginUtils_1.getErrorMessage)(page);
                    if (emailError) {
                        this.bot.logger.warn(this.bot.isMobile, 'LOGIN-CODE', `Email rejected: ${emailError} (attempt ${attempt}/${this.maxManualAttempts})`);
                        if (attempt === this.maxManualAttempts) {
