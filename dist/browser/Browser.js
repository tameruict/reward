...
class Browser {
    ...
    async createBrowser(account) {
        ...
        try {
            ...
            const certArgs = hasProxy
                ...
                : [];
            this.bot.logger.info(this.bot.isMobile, 'BROWSER', `Launching bundled patched Chromium (Edge UA) | headless: ${headless} | platform: ${process.platform} | proxy: ${hasProxy ? 'yes (TLS errors ignored)' : 'no (TLS validated)'}`);
        ...
        try {
            const session = (0, SessionStore_1.loadSession)(this.bot.config.sessionPath, account.email, this.bot.isMobile);
    ...
    async assertProxyReachable(proxy) {
        const { host, port } = (0, ProxyConfig_1.parseProxyConfig)(proxy);
        await new Promise((resolve, reject) => {
            ...
            socket.setTimeout(8000, () => finish(new Error(`Proxy endpoint is unreachable: connection timed out after 8000ms`)));
