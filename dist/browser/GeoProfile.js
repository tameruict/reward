...
exports.timezoneOffsetForZone = timezoneOffsetForZone;
exports.isSupportedCountry = isSupportedCountry;
exports.resolveGeoProfile = resolveGeoProfile;
...
function seededInt(seed, salt, mod) {
    const digest = (0, node_crypto_1.createHash)('sha256').update(`${seed}::geo::${salt}`).digest();
 ...
 */
function timezoneOffsetForZone(timeZone, at = new Date()) {
    try {
 ...
 */
function resolveGeoProfile(country, seed) {
    if (!country)
        return null;
    const key = country.trim().toUpperCase();
    if (key.length !== 2)
...
//# sourceMappingURL=GeoProfile.js.map