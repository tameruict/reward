...
class VisualSearch extends Workers_1.Workers {
    ...
    async activate(data) {
        ...
        if (!offer.reportable) {
            this.bot.logger.info(this.bot.isMobile, 'VISUAL-SEARCH', `Visual search already active (or not activatable) | offerId=${offer.offerId}`, 'green');
            return 'already-active';
        }
        if (!offer.hash) {
            this.bot.logger.warn(this.bot.isMobile, 'VISUAL-SEARCH', `Activation offer present but missing a hash | offerId=${offer.offerId}`);
            return 'failed';
        }
        ...
        try {
            ...
            if (acknowledged) {
                this.bot.logger.info(this.bot.isMobile, 'VISUAL-SEARCH', `Activated visual search | offerId=${offer.offerId}`, 'green');
                await this.bot.utils.wait(this.bot.utils.randomDelay(5000, 10000));
                return 'activated';
            }
            this.bot.logger.warn(this.bot.isMobile, 'VISUAL-SEARCH', `Activation not acknowledged | offerId=${offer.offerId} | status=${status}`);
            return 'failed';
        }
        catch (error) {
            this.bot.logger.error(this.bot.isMobile, 'VISUAL-SEARCH', `Activation error | offerId=${offer.offerId} | ${error instanceof Error ? error.message : String(error)}`);
            return 'failed';
        }
    }
...
//# sourceMappingURL=VisualSearch.js.map