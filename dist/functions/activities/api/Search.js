...
class Search extends Workers_1.Workers {
    async doSearch(isMobile) {
        const startBalance = Number(this.bot.userData.currentPoints ?? 0);
        let totalGained = 0;
        this.bot.logger.info(isMobile, 'SEARCH-BING', `Starting Bing searches | currentBalance=${startBalance}`);
        try {
            const missing = this.bot.browser.func.missingSearchPoints(await this.bot.browser.func.getSearchPoints(), isMobile);
            this.bot.logger.info(isMobile, 'SEARCH-BING', `Search points remaining | edge=${missing.edgePoints} | desktop=${missing.desktopPoints} | mobile=${missing.mobilePoints}`);
            ...
            while (performed < MAX_SEARCHES) {
                ...
                if (res.balance != null)
                    this.bot.userData.currentPoints = res.balance;
                ...
                if (searchProgress > 0) {
                    stagnant = 0;
                    this.bot.logger.info(isMobile, 'SEARCH-BING', `pointsGained=${gained} | currentBalance=${res.balance} | query="${query}" | searchPts=${cap}`, 'green');
                }
                else {
                    stagnant++;
                    this.bot.logger.info(isMobile, 'SEARCH-BING', `No points gained ${stagnant}/${STAGNANT_LIMIT} | query="${query}" | searchPts=${cap}`);
                }
                if (capReached) {
                    this.bot.logger.info(isMobile, 'SEARCH-BING', `Search point cap reached (${cap}), stopping`, 'green');
                    break;
                }
                if (stagnant >= STAGNANT_LIMIT) {
                    this.bot.logger.warn(isMobile, 'SEARCH-BING', `No points for ${STAGNANT_LIMIT} searches in a row, aborting`);
                    break;
                }
            ...
            this.bot.logger.info(isMobile, 'SEARCH-BING', `Completed Bing searches | pointsGained=${totalGained} | currentBalance=${this.bot.userData.currentPoints} | previousBalance=${startBalance} | searches=${performed}`);
            return totalGained;
        }
        catch (error) {
            this.bot.logger.error(isMobile, 'SEARCH-BING', `Error in doSearch | ${error instanceof Error ? error.message : String(error)}`);
            return totalGained;
        }
    }
    async doBonusSearches() {
        ...
        try {
            ...
            while (!tracker.done() && performed < tracker.maxSearches && stagnant < tracker.stagnantLimit) {
                ...
                if (gained > 0) {
                    ...
                    stagnant++;
                    this.bot.logger.info(isMobile, tracker.context, `no points ${stagnant}/${tracker.stagnantLimit} | query="${query}" | ${tracker.progress()}`);
                }
                await this.bot.utils.wait(this.bot.utils.randomDelay(this.bot.config.searchSettings.searchDelay.min, this.bot.config.searchSettings.searchDelay.max));
            }
        }
        catch (error) {
            this.bot.logger.error(isMobile, tracker.context, `Bonus session error | ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        this.bot.logger.info(isMobile, tracker.context, `Bonus farming ${done ? 'complete' : 'stopped'} (${reason}) | pointsGained=${totalGained} | currentBalance=${this.bot.userData.currentPoints} | ${tracker.progress()} | searches=${performed}`, done || totalGained > 0 ? 'green' : undefined);
        return totalGained;
    }
