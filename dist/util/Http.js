...
exports.ProxyUnavailableError = void 0;
...
function toInit(config) {
    let url = config.url ?? '';
    if (config.params) {
        const qs = config.params instanceof URLSearchParams
...
async function toResponse(res, config) {
    ...
    const withSetCookie = res.headers;
...
function proxyFailureReason(error) {
    const status = error?.status;
    if (status === 407)
        return 'proxy authentication was rejected';
...
async function send(instance, url, init, config, retries) {
    let attempt = 0;
    for (;;) {
        let res;
        try {
        ...
        const out = await toResponse(res, config);
...
class HttpClient {
    ...
    proxyUrl;
    proxyKey;
    lastProxyHealthcheck = 0;
    usesProxy;
    constructor(account) {
        this.account = account;
        this.proxyUrl = this.account.url.trim() ? (0, ProxyConfig_1.formatProxyUrl)(this.account) : undefined;
        this.proxyKey = this.proxyUrl ? this.buildProxyKey(this.account) : undefined;
        this.usesProxy = Boolean(this.proxyUrl);
        this.instance = this.createInstance();
    }
    async assertProxyReady(force = false) {
        if (!this.usesProxy)
            return;
        this.throwIfProxyCircuitOpen();
        if (!force && Date.now() - this.lastProxyHealthcheck < PROXY_HEALTH_TTL)
            return;
        let lastError;
        for (let attempt = 1; attempt <= PROXY_HEALTHCHECK_ATTEMPTS; attempt++) {
            try {
                ...
                if (response.status < 200 || response.status >= 300) {
                    const error = new Error(`Proxy health check returned HTTP ${response.status}`);
    ...
    async request(config) {
        ...
        try {
            ...
            return await send(this.instance, url, init, config, this.usesProxy ? 0 : 5);
        }
        catch (error) {
    ...
    createInstance() {
        return new impit_1.Impit({ browser: 'chrome', proxyUrl: this.proxyUrl, timeout: DEFAULT_TIMEOUT });
    }
    buildProxyKey(proxyConfig) {
        const parsed = (0, ProxyConfig_1.parseProxyConfig)(proxyConfig);
        return `${parsed.protocol}://${parsed.host}:${parsed.port}|${parsed.username}`;
    }
    throwIfProxyCircuitOpen() {
        if (!this.proxyKey)
            return;
    ...
    markProxyUnavailable(error) {
        ...
        const failureCount = existing && now - existing.lastFailureAt <= PROXY_FAILURE_WINDOW ? existing.failureCount + 1 : 1;
...
async function httpRequest(config) {
    if (!sharedInstance)
        sharedInstance = new impit_1.Impit({ browser: 'chrome', timeout: DEFAULT_TIMEOUT });
    const { url, init } = toInit(config);
    return send(sharedInstance, url, init, config, 0);
}
