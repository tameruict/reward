...
import {
    buildAllAccountsEnv,
    buildExcludedAccountsEnv,
    buildSingleAccountEnv,
    listConfiguredAccountRows,
    mergeAccountStats
} from './accounts.js'
import {
    assignAccountProxy,
...
const pointCheckProxyInFlight = new Map()
const pointCheckResults = new Map()

const startedAt = Date.now()

function listDashboardAccountRows() {
    const managed = listManagedAccountRows(projectRoot)
    return managed.length ? managed : listConfiguredAccountRows()
}
...
function pointCheckProxyKey(account) {
    const proxy = account.proxy ?? {}
    return String(proxy.id ?? `${proxy.url ?? ''}:${proxy.port ?? ''}`).trim()
}

function runPointCheckWorker(account, timeoutMs) {
...
function runPointCheck(account) {
    ...
    const proxyKey = pointCheckProxyKey(account)
    const previousOnProxy = pointCheckProxyInFlight.get(proxyKey)
    if (previousOnProxy) {
        pm.note('info', `Point check queued behind another account on proxy ${proxyKey}.`)
    }

    const promise = (previousOnProxy ? previousOnProxy.catch(() => {}) : Promise.resolve())
        .then(() => runPointCheckWorker(account, timeoutMs))
    const tracked = promise.finally(() => {
        if (pointCheckInFlight.get(accountId) === tracked) pointCheckInFlight.delete(accountId)
        if (pointCheckProxyInFlight.get(proxyKey) === tracked) pointCheckProxyInFlight.delete(proxyKey)
    })
    pointCheckInFlight.set(accountId, tracked)
    pointCheckProxyInFlight.set(proxyKey, tracked)
    return tracked
}
...
const requestHandler = async (req, res) => {
    ...
    try {
        ...
        if (method === 'GET' && pathname === '/accounts') {
            const accounts = mergeAccountStats(listDashboardAccountRows(), pm.getHistory().map(toHistoryRecord)).map(account => ({
                ...account,
        ...
        if (method === 'POST' && pathname === '/proxies') {
            ...
            // The Replit dashboard accepts a single proxy URL and does not
            // require users to invent a separate label. Generate a stable,
            // non-secret label so that the stored proxy can still be assigned
            // to accounts later.
            const proxyInput = { ...proxy }
            if (!String(proxyInput.label ?? '').trim()) {
                const identity = String(proxyInput.url ?? '').trim()
                proxyInput.label = `proxy-${crypto.createHash('sha1').update(identity).digest('hex').slice(0, 10)}`
            }
            const result = importAccountBundle(projectRoot, { proxies: [proxyInput], accounts: [] })
        ...
        if (method === 'POST' && pathname === '/accounts/import') {
            ...
            let bundle = body && body.bundle && typeof body.bundle === 'object' ? body.bundle : body

            // The Replit dashboard imports bare email/password rows first and
            // lets the user assign a stored proxy afterwards. Normalize only
            // that legacy UI shape; explicit API clients can still reject
            // direct accounts with allowDirectAccounts: false.
            const accounts = Array.isArray(bundle?.accounts) ? bundle.accounts : []
            const isLegacyWebImport =
                accounts.length > 0 &&
                (!Array.isArray(bundle?.proxies) || bundle.proxies.length === 0) &&
                !bundle.autoAssignStoredProxies &&
                bundle.allowDirectAccounts !== false &&
                accounts.every(
                    account =>
                        account &&
                        typeof account === 'object' &&
                        account.useProxy == null &&
                        !account.proxy &&
                        !account.proxyLabel &&
                        !account.proxy_label
                )
            if (isLegacyWebImport) {
                bundle = {
                    ...bundle,
                    allowDirectAccounts: true,
                    accounts: accounts.map(account => ({ ...account, useProxy: false }))
                }
            }
